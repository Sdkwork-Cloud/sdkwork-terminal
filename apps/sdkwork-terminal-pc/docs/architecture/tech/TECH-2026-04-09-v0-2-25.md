> Migrated from `docs/release/2026-04-09-v0.2.25.md` on 2026-06-24.
...
- Wave B
- Step 07 进行中：完成 Session Center replay timeline gap summary 子阶段

## Changed

- `packages/sdkwork-terminal-sessions/src/model.ts` 新增 `replayGap` 与 `summarizeSessionReplayGap`，让 Session Center 从 bounded replay slice 派生最大 timeline gap 摘要。
...
- 根 `package.json`、前端 apps/packages、Cargo workspace 与 `src-tauri/tauri.conf.json` 统一升级到 `0.2.25`。
...
- 回退到 `v0.2.24` 可恢复 Step 07 的 replay freshness projection 基线。
- 若继续推进 replay sequence gap、freshness drift 诊断或 interactive attach 主链，必须继续同步更新 `docs/架构`、`docs/step`、`docs/review` 与 `docs/release`。
