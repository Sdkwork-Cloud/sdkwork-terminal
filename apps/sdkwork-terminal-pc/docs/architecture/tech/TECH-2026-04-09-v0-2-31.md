> Migrated from `docs/release/2026-04-09-v0.2.31.md` on 2026-06-24.
...
- Wave B
- Step 07 进行中：完成 Session Center replay latest output projection 子阶段

## Changed

- `packages/sdkwork-terminal-sessions/src/model.ts` 新增 `replayLatestOutput` 与 `summarizeSessionReplayLatestOutput`，让 Session Center 直接从 replay `output` 事件派生最新终端输出摘要。
...
- 根 `package.json`、前端 apps/packages、Cargo workspace 与 `src-tauri/tauri.conf.json` 版本统一升级到 `0.2.31`。
...
- 当前 `SessionsPanel` 已能显示 bounded transcript、replay unavailable 诊断、replay cursor metadata、replay cursor drift summary、replay ack lag summary、replay ack window summary、replay window coverage summary、replay latest state projection、replay latest output projection、replay health summary、replay latest warning projection、replay latest exit projection、replay freshness projection、replay timeline gap summary、replay sequence gap summary、replay timeline metadata、replay event mix metadata 与 replay warning/exit evidence digest，但仍未提供 attach / detach / replay 游标控制入口。
- 远程目标 interactive terminal attach、实时 streaming 与 Remote Runtime broker / node 主链仍未闭环。
- Step 07 仍未达到 `L3` 完成定义。

## Rollback

- 回退到 `v0.2.30` 可恢复 Step 07 的 replay window coverage summary 基线。
