...
export type AiCliKind = "codex" | "claude-code" | "gemini" | "opencode";

export interface AiCliBinaryDiscovery {
  cliKind: AiCliKind;
  found: boolean;
...
export interface AiCliAuthState {
  authenticated: boolean;
  summary: string;
  details: string | null;
}

export interface AiCliLaunchTemplate {
  cliKind: AiCliKind;
  displayName: string;
  workdir: string | null;
  args: string[];
  ...
  recoveryPolicy: AiCliRecoveryPolicy;
  sessionTags: string[];
}

export interface AiCliRecoveryPolicy {
  autoRestart: boolean;
  maxRetries: number;
  preserveWorkdir: boolean;
}

export interface AiCliDiscoverySnapshot {
  platformFamily: string;
  cpuArch: string;
  discoveries: AiCliBinaryDiscovery[];
  checkedAt: string;
}

export interface AiCliSessionCreateRequest {
  cliKind: AiCliKind;
  workspaceId: string;
