...
use crate::error::{SessionRuntimeError, SessionRuntimeResult};
use crate::session::{
    AttachmentRecord, ConnectorLaunchResolution, ConnectorLaunchResolutionResult, ReattachResult,
    SessionCreateRequest, SessionRecord,
};
            ...
            if !matches!(session.state, SessionState::Exited | SessionState::Failed) {
                if !matches!(session.state, SessionState::Starting) {
                    let has_other_active = self
                        .attachments
                        .values()
                        .any(|a| a.session_id == attachment.session_id);
                    if !has_other_active {
...
mod tests {
    use super::*;
    use crate::daemon::{LocalDaemonPhase, LocalSessionDaemon};
    use crate::readiness_summary;
    use crate::session::{ConnectorLaunchResolution, SessionLaunchIntent};
    ...
    fn recovered_sqlite_runtime_normalizes_stale_non_terminal_sessions_to_exited() {
        ...
        assert_eq!(
            detached_replay.entries[0].payload,
            "detached session prompt"
        );
