use crate::channel::SessionEventSender;
use crate::constants::{
    MANAGED_TERMINAL_COLOR_CAPABILITY, MANAGED_TERMINAL_FORCE_COLOR_LEVEL,
    MANAGED_TERMINAL_PROGRAM, MANAGED_TERMINAL_PROGRAM_VERSION, MANAGED_TERMINAL_TYPE,
};
use crate::error::LocalShellExecutionError;
use crate::types::{
...
fn prepare_process_launch_command(
    ...
    #[cfg(windows)]
    {
        prepare_windows_process_launch_command(program, &command.args)
    }
...
fn prepare_windows_process_launch_command(
    ...
    match resolved_extension.as_deref() {
        Some("ps1") => {
            let launcher =
                resolve_windows_runtime_program(&["pwsh", "powershell"]).ok_or_else(|| {
        ...
        Some("cmd") => {
            if let Some(prepared) = prepare_windows_companion_powershell_shim_launch_command(
...
mod tests {
    ...
    fn managed_terminal_environment_sets_terminal_capabilities_on_command_builder() {
        ...
        assert_eq!(
            builder.get_env("CLICOLOR_FORCE"),
    ...
    fn executes_local_shell_command_and_preserves_non_zero_exit() {
        ...
        assert!(
            result.stderr.contains("sdkwork-failure") || result.stdout.contains("sdkwork-failure")
        );
    }
    ...
    fn session_runtime_creates_interactive_shell_and_streams_output() {
        ...
        let (sender, receiver) = crate::create_session_event_channel();
