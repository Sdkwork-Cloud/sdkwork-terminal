...
test("shared terminal viewport surface centralizes search overlay, host surface, host status, and context menu chrome", () => {
  ...
  assert.match(source, /searchStatus: TerminalSearchStatus;/);
  assert.match(source, /onSearchQueryChange: \(query: string\) => void;/);
  assert.match(source, /onSearchPrevious: \(\) => void;/);
  ...
  assert.match(source, /onRestoreViewportFocus: \(\) => Promise<void> \| void;/);
  assert.match(source, /pasteConfirmation: \{/);
  ...
  assert.match(source, /status=\{props\.searchStatus\}/);
  assert.match(source, /onPrevious=\{props\.onSearchPrevious\}/);
  assert.match(source, /<TerminalHostStatusOverlay/);
  assert.match(source, /<TerminalPasteConfirmationDialog/);
  ...
  assert.match(source, /onRestoreFocus=\{props\.onRestoreViewportFocus\}/);
